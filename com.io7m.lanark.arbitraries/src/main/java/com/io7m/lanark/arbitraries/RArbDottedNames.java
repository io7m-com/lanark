...
/**
 * Arbitrary restricted dotted names.
 */

public final class RArbDottedNames implements ArbitraryProvider
{
  /**
   * Arbitrary restricted dotted names.
   */

  public RArbDottedNames()
  {
  ...
  {
    return targetType.isOfType(RDottedName.class);
  }
    ...
    final SubtypeProvider subtypeProvider)
  {
    ...
    return Set.of(
      Combinators.combine(primarySegment, secondarySegments)
        .as((start, end) -> {
          if (end.isEmpty()) {
            return new RDottedName(start);
          } else {
            return new RDottedName("%s.%s".formatted(start, end));
          }
        })
    );
  }
